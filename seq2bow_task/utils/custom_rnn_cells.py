    ...
    with tf.variable_scope(scope) as outer_scope:
        ...
        with tf.variable_scope(outer_scope) as inner_scope:
            ...
            biases = tf.get_variable(
                "linear_bias", [output_size], dtype=dtype,
...
class RANCell(tf.nn.rnn_cell.RNNCell):
    def __init__(self, num_units):
        self._num_units = num_units
        self._activation = tf.tanh  # use tanh as output
    ...
    def __call__(self, inputs, state, scope=None):
        inputs_size = inputs.get_shape()[1].value

        with tf.variable_scope(scope or type(self).__name__):
            with tf.variable_scope("Candidate"):
                if inputs_size != self._num_units:
                    input_proj = tf.get_variable(
                        "input_projection",
                        [inputs_size, self._num_units],
                        dtype=tf.float32)
                    _candidate = tf.matmul(inputs, input_proj)
                else:
                    _candidate = inputs

            with tf.variable_scope("Gates"):
                i, f = tf.split(1, 2, _linear([inputs, state],
                                2 * self._num_units, True, 1.0))
                i, f = tf.sigmoid(i), tf.sigmoid(f)


                new_state = i * _candidate + f * state
